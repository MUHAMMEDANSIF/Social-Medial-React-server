...
export const Login = async (req, res, next) => {
  try {
    ...
    res.json({
      error: 'Some technical Problem Please try after some time we will fix it', message: err.message,
    });
  }
};
...
export const sendotp = async (req, res) => {
  try {
    ...
    const email_checking = await userSchema.findOne({ email });
    if (email_checking) {
      const options = {
        min: 100000,
        max: 999999,
      ...
      transporter.sendMail(info, async (error) => {
        if (error) {
          console.log('error');
          console.log(error);
          res.json({ error: true });
...
export const forgottenpassword = async (req, res) => {
  try {
    if (req.body.password || req.body.email) {
      ...
      const password_reset = await userSchema.findOneAndUpdate({ email }, {
        $set: {
          password,
        },
      });
